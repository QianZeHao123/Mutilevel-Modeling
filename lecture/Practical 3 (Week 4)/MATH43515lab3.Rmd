...
$$y_{ij}=a + b x_{ij} + u_j +v_j x_{ij} +\epsilon_{ij} $$
where $u_j\sim N(0,\sigma^2_u)$, $v_j \sim N(0,\sigma^2_v)$ and the independent error terms are $\epsilon_{ij} \sim N(0,\sigma^2)$.
...
$$a_j = a + \alpha \text{exp}_j +u_j$$
where $u_j\sim N(0,\sigma^2_u)$. Hence, the full model becomes

$$y_{ij}=a +\alpha \text{exp}_j + b x_{ij} + u_j + \epsilon_{ij} $$ 
We can fit this model in R via:
```{r}
