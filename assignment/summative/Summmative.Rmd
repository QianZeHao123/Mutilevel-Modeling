...
1.  Using the MST_long (long format) to fit an empty GLMM
    ...
    init_model_long <- glmer(
      StressScore ~ Trt + as.numeric(TimeStamp) + (1 | Hospital:ID) + (1 | Hospital),
      data = MST_long,
      family = gaussian(link = "identity")
      # family = poisson(link = "log")
    )
    # empty_model_long <- glmer(
    #   StressScore ~ 1 + (1 | Hospital),
    #   data = MST_long,
    #   family = poisson(link = "log")
    # )
    summary(init_model_long)
    ```

    ```{r}
    empty_model_long.2 <- glmer(
      StressScore ~ Trt + Trt:TimeStamp + TimeStamp + (1 | Hospital:ID) + (1 | Hospital),
      data = MST_long,
      family = gaussian(link = "identity")
      # family = poisson(link = "log")
    )

    empty_model_long.3 <- glmer(
      StressScore ~  Trt*TimeStamp + (1 | Hospital:ID) + (1 | Hospital),
      data = MST_long,
      family = gaussian(link = "identity")
      # family = poisson(link = "log")
    )

    empty_model_long.4 <- glmer(
      StressScore ~  Trt*TimeStamp + c(Trt*Trt) + (1 | Hospital:ID) + (1 | Hospital),
      data = MST_long,
      family = gaussian(link = "identity")
      # family = poisson(link = "log")
    )
    ...
    ```{r}
    summary(empty_model_long.2)
    ```

    ```{r}
    summary(empty_model_long.3)
    ```

2.  empty_model_long.1 \<- glmer( StressScore \~ Trt + Trt:TimeStamp + (1 \| Hospital:ID) + (1 \| Hospital), data = MST_long, family = gaussian(link = "identity") \# family = poisson(link = "log") )Decompose Variance
...
![](http://127.0.0.1:15441/chunk_output/s/726218B8/ch7wyo1ti9wp4/00001a.png)
